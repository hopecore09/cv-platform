    ...
    <div>
      ...
      <Card className="mb-4"><Card.Body>
        <Form onSubmit={handleSubmit(onSubmit)}>
          <Row>
            <Col md={6}><Form.Group><Form.Label>{t('profile.firstName')}</Form.Label><Form.Control {...register('firstName')} /></Form.Group></Col>
            <Col md={6}><Form.Group><Form.Label>{t('profile.lastName')}</Form.Label><Form.Control {...register('lastName')} /></Form.Group></Col>
          </Row>
      ...
      {/* Мои CV */}
      <Card className="mt-4">
        <Card.Header><h5 className="mb-0">My CVs</h5></Card.Header>
        <Card.Body>
          {profile?.cvs?.length ? (
            <div className="list-group">
              {profile.cvs.map(cv => (
                <Link key={cv.id} to={`/cv/${cv.id}`} className="list-group-item list-group-item-action d-flex justify-content-between align-items-center">
                  {cv.position?.title}
                  <span className={`badge ${cv.isPublished ? 'bg-success' : 'bg-warning'}`}>
